...
static const vertex_t sprite_vertices[] = {
  { .pos = { +0.5, +0.5, 0.5 }, .uv = { 1.0f, 0.0f } },
  { .pos = { +0.5, -0.5, 1.0 }, .uv = { 1.0f, 1.0f } },
  { .pos = { -0.5, +0.5, 0.0 }, .uv = { 0.0f, 0.0f } },
...
void sprite_mesh_draw(mesh_t *sprite_mesh, const sprite_t sprites[MAX_SPRITES], const camera_t *camera)
{
  ...
  for (int i = 0; i < MAX_SPRITES; i++) {
    ...
    mat4x4_t rot_mat = mat4x4_mul(stand_mat, mat4x4_init_rotation(camera->rot));
    mat4x4_t pos_mat = mat4x4_init_translation(pos);
    
    mat4x4_t vert_mat = mat4x4_mul(rot_mat, pos_mat);
    
    for (int j = 0; j < num_sprite_vertices; j++) {
      vec3_t pos = mat4x4_mul_vec3(vert_mat, sprite_vertices[j].pos);
      vec2_t uv = vec2_mulf(vec2_add(sprite_vertices[j].uv, vec2_init(0.0, 1.0)), size_uv);
